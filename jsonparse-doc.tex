...
\def\jsonparsefileversion{1.8.0}
\def\jsonparsefiledate{11 February 2026}
...
\changes{v1.8.0}{2026/02/11}{Added option to make indices of array items accessible via key.}
...
The command \macro{\JSONParseArrayUse} also accepts as option set in the optional argument the key \macro{zero-based} to set the initial value of the mapping counter to the correct value if the key \macro{zero-based} is not set globally.
...
The command \macro{\JSONParseArrayMapFunction} accepts as option set in the optional argument the key \macro{zero-based} to set the initial value of the mapping counter to the correct value if the key \macro{zero-based} is not set globally.
...
The command \macro{\JSONParseArrayMapInline} accepts as option set in the optional argument the key \macro{zero-based} to set the initial value of the mapping counter to the correct value if the key \macro{zero-based} is not set globally.
...
\begin{macrodef}
|provide index|
|provide index|={<boolean>}
\end{macrodef}
If the key \macro{provide index} is set (or explicitly set to \macro{true}), the indices of array items are stored in the parsed JSON data and can be accessed using the key as set with \macro{index key} which is \macro{_} per default.

For example, assuming the same JSON data parsed into the token variable \macro{\myJSONdata} as in the examples above, calling \macro{\JSONParseValue{\myJSONdata}{array[0]._}} would result in `0'.

This key can be set using \macro{\JSONParseSet}. It can also be set locally as option to the commands \macro{\JSONParse} and \macro{\JSONParseFromFile}. When set using \macro{\JSONParseSet}, these keys only take effect when set before parsing.

\begin{macrodef}
|index key|={<string>}
\end{macrodef}
With the key \macro{index key}, the string that is used as key for the index of a value in an array can be changed if the key \macro{provide index} is used. The default string used as key is \macro{_}. 
...
It is, for example, possible to make use of this mechanism by defining a new plug to parse JSONL files which consist of a number of JSON objects separated by a line break (the two lines with regular expression replacements ensure that no unwanted leading or trailing commas appear in the array):
...
|preparse|={<string>}
\end{macrodef}
The commands \macro{\JSONParse} and \macro{\JSONParseFromFile} accept the key \macro{preparse} as option. The key takes as argument the name of the plug that will be assigned to the \macro{jsonparse/preparse} or \macro{jsonparse/preparse-file} socket and then used to parse the relevant JSON string or file.

To assign the plug for all following calls of \macro{\JSONParse} or \macro{\JSONParseFromFile}, assign the plug use the command \macro{\AssignSocketPlug{jsonparse/preparse}{jsonl}} or the command \macro{\AssignSocketPlug{jsonparse/preparse-file}{jsonl}}.
