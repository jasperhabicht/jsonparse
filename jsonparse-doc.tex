...
\end{macrodef}
If the key \macro{store index} is set (or explicitly set to \macro{true}), the indices of array items are stored in the parsed JSON data and can be accessed using the key as set with \macro{index key} which is \macro{_} per default.
