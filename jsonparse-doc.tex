...
The command \macro{\JSONParseArrayMapInline} accepts the key \macro{store in} which takes a token list into which the result of the mapped inline function should be stored. It is important to note that the inline function needs to be fully expandable in order to be able to be stored in the given token list. For example, it is not possible to use \macro{\JSONParseValue} in the code of the inline function while \macro{\JSONParseExpandableValue} is allowed.

Storing the result of the mapped inline function can be helpful if JSON data should be reformatted in order to feed the reformatted data to plotting functions. An example for a use case with PGFplots is shown below. In this example, the token variable \macro{\myJSONplotdata} contains the parsed JSON string \macro{{ "data": [ [0,0], [1,-1], [2,1] ] }}.
...
\begin{tikzpicture}
\begin{axis}
\addplot coordinates {\myJSONcoords};
\end{axis}
\end{tikzpicture}
\end{codeexamplecolumns}
\fi
