...
|store index|={<boolean>}
\end{macrodef}
If the key \macro{store index} is set (or explicitly set to \macro{true}), the indices of array items are stored in the parsed JSON data and can be accessed using the key as set with \macro{keyword/index} which is \macro{_} per default.
...
With the key \macro{keyword/index}, the string that is used as key for the index of a value in an array can be changed if the key \macro{store index} is used. The default string used as key is \macro{_}.
